...
function CardTitle({
  ...
  return (
    <h3
      className={cn(
        "overflow-wrap-anywhere font-headline font-bold leading-[1.5] line-clamp-2",
        className,
      )}
    >
      {children}
...
function CategoryBadge({ slug }: { slug: string }) {
  ...
  return (
    <span
      className={cn(
        "inline-block rounded px-2 py-[2px] text-[10px] font-bold uppercase tracking-[0.08em]",
...
export function ArticleCard({ article, variant = "default", index }: ArticleCardProps) {
  ...
  if (variant === "numbered") {
    return (
      <Link href={`/article/${article.slug}`} className="group flex items-start gap-4 py-3">
  ...
  if (variant === "sidebar") {
    return (
      <Link href={`/article/${article.slug}`} className="group flex items-start gap-3 py-2">
